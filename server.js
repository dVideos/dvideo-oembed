...
const rootDomain = 'https://dvideo.io'
...
app.get('*', function(req, res, next) {
    ...
    if (reqPath.startsWith('/oembed')) {
        ...
        if (req.query.url && isValidDVideoUrl(req.query.url) ) {
            getVideo(
            ...
            function(err, html, pageTitle, description, url, snap, urlvideo, duration) {
                ...
                var response = {
                    type: 'video',
                    version: '1.0',
                    provider_name: 'dVideo',
                    provider_url: rootDomain,
                    title: pageTitle,
                    author_name: req.query.url.split('/')[4],
                    author_url: 'https://dvideo.io/#!/c/'+req.query.url.split('/')[4],
                    html: html,
                    width: sizes.width,
                    height: sizes.height,
...
function getVideo(author, permlink, cb) {
    lightrpc.send('get_state', [`/dvideo/@${author}/${permlink}`], function(err, result) {
        if (err) {
            cb(err)
            return
        }
        ...
        var html = '<iframe width="480" height="270" src="https://get.dvideo.io/#!/'+author+'/'+permlink+'" frameborder="0" allowfullscreen></iframe>'
